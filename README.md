...
## Rocky Linux / CentOS SQLite Fix

ChromaDB requires SQLite `3.35.0` or higher. Some Rocky Linux, CentOS, and RHEL systems use an older SQLite library inside Python. This repo fixes that by loading `pysqlite3-binary` before ChromaDB imports.
...
Expected output should show SQLite `3.35.0` or higher, usually from `pysqlite3-binary`.

If you still get the SQLite error, recreate the virtual environment:
...
Do not import `chromadb` in `src/__init__.py` or before `from src import compat` runs.
